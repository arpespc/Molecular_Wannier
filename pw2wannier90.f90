...
module wannier
   ...
   !! < molecular orbital projections related 
   logical  :: have_molecular  ! FIXME: zh_mo: check whether have molecular projections
   integer  :: n_molecular ! FIXME: zh_mo: number of molecular orbitals projections, 
   integer  :: n_mobasis ! FIXME: zh_mo: for each molecular orbitals, the number of basis which construct the molecular orbital
   integer  :: n_mo_entry ! FIXME: zh_mo: the number of line in nnkp file diffine the molecular orbitals, this seems no need to define here, but in read_nnkp is enough
   !! The number of molecular orbits (n_mobasis) must equals to n_proj
   !!  /molecular orital projections related>

   integer  :: n_proj    !number of projection
   complex(DP), allocatable :: gf(:,:)  ! guding_function(npwx,n_wannier)
   complex(DP), allocatable :: gf_mo(:,:)  ! FIXME: zh_mo: guiding fucntion of MO
   integer,  allocatable :: spin_eig_mo(:) ! FIXME: zh_mo: spin index for molecular orbital
                                           ! in principle, it is not nessecery to write it
                                           ! explicityly, just for conviniece for me.
   ! FIXME: zh_mo: coefficients of guiding fucntion of MO (in basis of atom orbitals defined in projections)
   complex(DP), allocatable :: mo_coefficents(:,:)  ! zh_mo (iatom,n_molecular)
   ! real(DP)   , allocatable :: mo_coefficents_tmp(:,:)  ! zh_mo (2*iatom, n_molecular)
...
SUBROUTINE read_nnkp
  ...
  IF (ionode) THEN   ! read from ionode only
     ...
     READ(iun_nnkp,*) n_proj !! number of projectors
  ENDIF
...
!  CALL errore("pw2wannier90","Spinorbit without spinor=T",1)
  ...
  ALLOCATE( center_w(3,n_proj), alpha_w(n_proj), gf(npwx,n_proj), &   !! alpha_w: zona in wannier90, gf: guiding function
       l_w(n_proj), mr_w(n_proj), r_w(n_proj), &                      !! l_w: l and mr and index of ratil function  of wannier (n_wannier) as from table 3.3 of spec.
       zaxis(3,n_proj), xaxis(3,n_proj), csph(16,n_proj) )            !! expansion coefficients of gf on QE ylm function (16, n_wannier).
  ...
  IF (ionode) THEN   ! read from ionode only
     DO iw=1,n_proj
        ...
        CALL cryst_to_cart( 1, center_w(:,iw), at, 1 )
        if(noncolin.and..not.old_spinor_proj) then
           !! for example of C orbotals, 
           !! pz 1
           !! pz -1
           !! px 1
           !! px -1 .... where 1 and -1 are the spin_eig(iw)
  ...
  ! FIXME: zh_mo: define molecular orbitals projections
  IF (ionode) THEN 
     CALL scan_file_to('molecular_orbitals', found)
     IF (.not. found) THEN
        WRITE(stdout, *) 'NO PROJECTION OF MOLECULAR ORBITALS'
     ELSE
        have_molecular = .TRUE.
        WRITE(stdout, *) 'PROJECTION OF MOLECULAR ORBITALS'
        READ (iun_nnkp, *) n_molecular, n_mobasis, n_mo_entry
        WRITE(stdout, *) 'number of molecular orbitals: ', n_molecular
        WRITE(stdout, *) 'number of molecular orbitals basis (i.e. wannier projector): ', n_mobasis
        WRITE(stdout, *) 'number of lines define the molecular orbitals: ', n_mo_entry 

        !! zh_mo: the number of basis to construct the molecular orbital should be the same as the number of projector
        !! i.e. Molecular_orbital_1 = \sum_{i=1}^{n_proj} coefficient(1, i) * projector(i)
        IF ( (n_mobasis /= n_proj)) THEN 
           CALL errore('read_nnkp', 'Error: the number of basis to construct a molecular should be the same as the number of projector', 1)
        ENDIF

        !! allocate
      !  ALLOCATE(gf_mo(npwx, n_molecular), stat=allocate_stat) ! zh_mo: allocate gf_mo
      !  IF (allocate_stat > 0) THEN
      !     CALL errore('read_nnkp', 'allocate gf_mo fails', 1) 
      !  ENDIF
      !  ALLOCATE(mo_coefficents_tmp(2*n_mobasis, n_molecular), stat=allocate_stat) ! zh_mo: read the coefficient now, and convert it to complex number later
      !  IF (allocate_stat > 0) THEN
      !     CALL errore('read_nnkp', 'allocate mo_coefficients_tmp fails', 1) 
      !  ENDIF
      !  ALLOCATE(mo_coefficents(n_mobasis, n_molecular), stat=allocate_stat)
      !  IF (allocate_stat > 0) THEN
      !     CALL errore('read_nnkp', 'allocate mo_coefficients fails', 1) 
      !  ENDIF
      !  WRITE(*,*) 'all molecular allocate successfully'

      !  ! FIXME: 
      !  ! zh_mo: the real wannier function is not the atoms orbitals anymore, but the 
      !  ! molecular orbital, which is the liear comnbination of the atoms orbitals defined
      !  ! in the ``projections``  input card
      !  ! n_proj = n_molecular 
      !  DO i_mo = 1, n_molecular
      !     READ(iun_nnkp, *) mo_coefficents_tmp(:, i_mo)
      !     WRITE(*,*) "READ I_MO : ", i_mo
      !     DO i_mobase = 1, n_mobasis
      !        mo_coefficents(i_mobase, i_mo) = CMPLX(mo_coefficents_tmp(i_mobase*2 - 1, i_mo), mo_coefficents_tmp(i_mobase*2, i_mo), KIND=DP)
      !     ENDDO  ! zh_mo: end n_basis
      !  ENDDO ! zh_mo: end n_molecular
      !  WRITE(*,*) 'READ MOLECULAR PROJECTION FINISHED'
     ENDIF
  ENDIF 

  CALL mp_bcast(have_molecular, ionode_id, world_comm)
  CALL mp_bcast(n_molecular, ionode_id, world_comm)
  CALL mp_bcast(n_mobasis, ionode_id, world_comm)
  CALL mp_bcast(n_mo_entry, ionode_id, world_comm)

  ALLOCATE(gf_mo(npwx, n_molecular), stat=allocate_stat) 
  IF (allocate_stat > 0) THEN
     CALL errore('read_nnkp', 'Error with allocate gf_mo', 1)
  ENDIF

  IF (noncolin) THEN
     ALLOCATE(spin_eig_mo(n_molecular), stat=allocate_stat) 
     IF (allocate_stat > 0) THEN
        CALL errore('read_nnkp', 'Error with allocate spin_eig_mo', 1)
     ENDIF
  ENDIF

  ! ALLOCATE(mo_coefficents_tmp(2*n_mobasis, n_molecular), stat=allocate_stat) 
  ! IF (allocate_stat > 0) THEN
  !    CALL errore('read_nnkp', 'Error with allocate mo_coefficents_tmp', 1)
  ! ENDIF

  ALLOCATE(mo_coefficents(n_mobasis, n_molecular), stat=allocate_stat) 
  IF (allocate_stat > 0) THEN
     CALL errore('read_nnkp', 'Error with allocate mo_coefficents', 1)
  ENDIF

  ! gf_mo(:,:) = 0.0d0
  ! mo_coefficents_tmp(:,:) = (0.0d0, 0.0d0)
  DO i_mo = 1, n_molecular
     DO i_mobase = 1, n_mobasis
        ! this is not efficient, but it will not affect too much, 
        ! and i'm not willing to google it now
        mo_coefficents(i_mobase, i_mo) = CMPLX(0.0d0, 0.0d0, KIND=DP)
     ENDDO
  ENDDO
  ...
  IF (ionode) THEN
     IF (have_molecular) THEN
        IF (noncolin) THEN
           ...
           WRITE(stdout, *) 'All molecular allocated successfully'
           WRITE(stdout, *) 'The defination of molecular orbitals is: '
           WRITE(stdout, *) 'i_mo, i_mobase, i_spin, coeff_real, coeff_imag'
           DO i_entry = 1, n_mo_entry
              READ(iun_nnkp, *) i_mo, i_mobase, i_spin, mo_coeff_real, mo_coeff_imag
              IF ( (i_mo < 0) .or. (i_mo > n_molecular) ) THEN 
                 CALL errore('read_nnkp', 'Error:  index of molecular orbital out of range', 1)
              ENDIF 
              IF ( (i_mobase < 0) .or. (i_mobase > n_mobasis) ) THEN 
                 CALL errore('read_nnkp', 'Error:  index of molecular orbital coefficent out of range', 1)
              ENDIF 

              WRITE(stdout, *)  i_mo, i_mobase, i_spin, mo_coeff_real, mo_coeff_imag
              mo_coefficents(i_mobase, i_mo) = CMPLX(mo_coeff_real, mo_coeff_imag, KIND=DP)
              spin_eig_mo(i_mo) = i_spin
           ENDDO
        ...
        ELSE
           ...
           WRITE(stdout, *) 'All molecular allocated successfully'
           WRITE(stdout, *) 'The defination of molecular orbitals is: '
           WRITE(stdout, *) 'i_mo, i_mobase, coeff_real, coeff_imag'
           DO i_entry = 1, n_mo_entry
              READ(iun_nnkp, *) i_mo, i_mobase, mo_coeff_real, mo_coeff_imag
              IF ( (i_mo < 0) .or. (i_mo > n_molecular) ) THEN 
                 CALL errore('read_nnkp', 'Error:  index of molecular orbital out of range', 1)
              ENDIF 
              IF ( (i_mobase < 0) .or. (i_mobase > n_mobasis) ) THEN 
              ...
              WRITE(stdout, *)  i_mo, i_mobase, mo_coeff_real, mo_coeff_imag
              mo_coefficents(i_mobase, i_mo) = CMPLX(mo_coeff_real, mo_coeff_imag, KIND=DP)
           ENDDO
        ENDIF
  ...
  CALL mp_bcast(mo_coefficents, ionode_id, world_comm)
  IF (noncolin) THEN
     CALL mp_bcast(spin_eig_mo, ionode_id, world_comm)
  ENDIF
  ! CALL mp_bcast(mo_coefficents_tmp, ionode_id, world_comm)
...
SUBROUTINE compute_amn
   ...
   INTEGER :: i_mo, i_base !! zh_mo: index for molecular orbitals and atoms' basis
   ...
   IF (wan_mode=='standalone') THEN
      ...
      IF (ionode) THEN
         ...
         IF (have_molecular) THEN
            WRITE (iun_amn,*) nbnd-nexband,  iknum, n_molecular
         ELSE
...
!      end if
      ...
      ! zh_mo: after here I can change the gf array

      !! < zh_mo : Change guiding functions gf here
      IF (have_molecular) THEN 
         DO i_mo = 1, n_molecular
            gf_mo(:, i_mo) = (0.d0, 0.d0)
            DO i_base = 1, n_mobasis
               !! The number of molecular orbits (n_mobasis) must equals to n_proj
               !! gf(npwx, n_molecular), mo_coefficents(n_basis, n_molecular), gf(npwx, n_proj)
               gf_mo(:, i_mo) = gf_mo(:, i_mo) + mo_coefficents(i_base, i_mo) * gf(:, i_base)
            ENDDO
         ENDDO
      ENDIF
      !! / Changing guiding functions gf>
      ...
      IF(any_uspp) THEN
         ...
         ! FIXME: zh_mo: do not suport uspp now
         IF (have_molecular) THEN
            WRITE (*,*) 'uspp is: ', any_uspp
            CALL errore('compute_amn', 'The uspp is not compatible with molecular orbitals projection now', 1)
         ENDIF
         ! end zh_mo
      ...
      ELSE
         sgf(:,:) = gf(:,:) ! noncolinear case. sgf = gf
      ENDIF
      !

      ! here, begining to calculate and write amn marix elements at each k point
      ...
      IF(noncolin) THEN
         ...
         IF(old_spinor_proj) THEN
            IF (have_molecular) THEN 
               CALL errore('compute_amn', 'do not support molecular orbital with old_spinor_proj_case', 1)
            ENDIF
            ! we do the projection as g(r)*a(r) and g(r)*b(r)
            DO ipol=1,npol
               istart = (ipol-1)*npwx + 1  ! npwx : maximal plane wavefunctions for wavefunctions
         ...
         ELSE old_spinor_proj_case ! not the old spinor case
            IF (have_molecular) THEN
               DO i_mo = 1, n_molecular
                  ...
                  !!! > 
                  !! zh_mo:  for simplicity, here we supose that the quantisation axis is along z
                  !! which is the most of the case
                  !!! <
                  ...
                  if(spin_z_pos .or. spin_z_neg) then
                     ...
                     DO ibnd = 1,nbnd
                        IF (excluded_band(ibnd)) CYCLE
                        if(spin_z_pos) then
                           ipol=(3-spin_eig_mo(i_mo))/2
                           !! if +1, ipol = (3 - 1 ) / 2 = 1
                           !! if -1, ipol = (3 - -1) / 2 = 2
                        else
                           CALL errore('compute_amn: ', 'Error, only support spin_z_pos case, but you can change it easyly', 1)
                           ipol=(3+spin_eig_mo(i_mo))/2
                        endif
                        istart = (ipol-1)*npwx + 1
                        !! if ipol = 1, istart = 1
                        !! if ipol = 2, istart = npwx + 1
                        amn=(0.0_dp,0.0_dp)
                        if (any_uspp) then
                           IF (have_molecular) THEN
                              CALL errore('compute_amn', 'Error: do not support molecular projection and uspp', 1)
                           ENDIF 
                           ! amn = zdotc(npw, evc(1, ibnd), 1, sgf_spinor(1, iw), 1)
                           ! amn = amn + zdotc(npw, evc(npwx+1, ibnd), 1, sgf_spinor(npwx+1, iw), 1)
                        else
                           
                           amn = zdotc(npw,evc(istart,ibnd),1,gf_mo(1,i_mo),1)
                        endif
                        CALL mp_sum(amn, intra_pool_comm)

                        ibnd1=ibnd1+1
                        IF (wan_mode=='standalone') THEN
                           IF (ionode) WRITE(iun_amn,'(3i5,2f18.12)') ibnd1, i_mo, ik, amn
                        ELSEIF (wan_mode=='library') THEN
                           IF (have_molecular) THEN
                              CALL errore('compute_amn', 'Error: do not support molecular orbitals and libarary mode', 1)
                           ENDIF 
                           ! a_mat(ibnd1,iw+n_proj*(ipol-1),ik) = amn
                        ELSE
                           CALL errore('compute_amn',' value of wan_mode not recognised',1)
                     ...
                     !! If  have molecular, this block of code will not be excutaed for now
                     !! so i didn't change it now.
                     ...
                     DO ibnd = 1,nbnd
                        ...
                        DO ipol=1,npol
                           ...
                           if (any_uspp) then
                             IF (have_molecular) THEN
                               CALL errore('compute_amn', 'Error: do not support uspp and molecular orbtal now', 1)
                             ENDIF
               ...
               DO iw = 1,n_proj
                  ...
                  if(spin_z_pos .or. spin_z_neg) then
                     ...
                     DO ibnd = 1,nbnd
                        ...
                        ELSEIF (wan_mode=='library') THEN
                           IF (have_molecular) THEN
                              CALL errore('compute_amn', 'Error: do not support molecular orbital projections and libarary mode')
                           ENDIF
                           a_mat(ibnd1,iw+n_proj*(ipol-1),ik) = amn
                        ELSE
                           CALL errore('compute_amn',' value of wan_mode not recognised',1)
            ...
            IF (have_molecular) THEN
               DO i_mo = 1,n_molecular
                  ...
                  DO ibnd = 1,nbnd
                     IF (excluded_band(ibnd)) CYCLE
                     IF (gamma_only) THEN
                        CALL errore('compute_amn', 'Error: do not surpport gamma only case by now', 1)
                        amn = 2.0_dp*ddot(2*npw,evc(1,ibnd),1, gf_mo(1,i_mo),1)
                        IF (gstart==2) amn = amn - real(conjg(evc(1,ibnd))*gf_mo(1,i_mo))
                     ELSE
                        amn = zdotc(npw,evc(1,ibnd),1, gf_mo(1,i_mo),1)
                     ENDIF
                     CALL mp_sum(amn, intra_pool_comm)
                     ibnd1=ibnd1+1
                     IF (wan_mode=='standalone') THEN
                  ...
                  ENDDO
               ENDDO
            ELSE ! zh_mo: do not have molecular orbitals
               DO iw = 1,n_proj
                  ibnd1 = 0
                  DO ibnd = 1,nbnd
                     IF (excluded_band(ibnd)) CYCLE
...
SUBROUTINE compute_amn_with_scdm
   ...
   DO ik=1,iknum
      ...
      DO iw = 1, n_wannier
        ...
        ! phase = cos(2*pi* cpos.k) + i sin(2*pi* cpos.k)
     ...
     CALL mp_bcast(VTmat,ionode_id,world_comm)
      ...
      ! Umat n_bands x n_wannier, VTmat: n_wannier x n_wanner
      DO iw = 1,n_wannier
         locibnd = 0
         DO ibnd = 1,nbtot
            IF (excluded_band(ibnd)) CYCLE
...
SUBROUTINE generate_guiding_functions(ik)
   !! zh_mo: whether or not the noncolinear case, the 
   !
   USE io_global,  ONLY : stdout
   USE constants, ONLY : pi, tpi, fpi, eps8
   USE control_flags, ONLY : gamma_only
   USE gvect, ONLY : g, gstart 
   ! G-vectors cartesian components ( in units tpiba =(2pi/a)  ), ALLOCATE( g(3, ngm) ), ! local  number of G vectors (on this processor)
   USE cell_base,  ONLY : tpiba ! 2 pi / a
   USE wannier
   USE klist,      ONLY : xk, ngk, igk_k !! xk: coordinate of k points, ngk: number of plane waves for each k point, igk_k: index of G corresponding to a given index of k+G
   ...
   !
   npw = ngk(ik) !! numer of plane wave function at each k points
   ALLOCATE( gk(3,npw), qg(npw), ylm(npw,lmax2), sk(npw), radial(npw,0:lmax) )

   ! ylm 第 ik 个k 点的npw个平面波
   !
   DO ig = 1, npw
      gk (1,ig) = xk(1, ik) + g(1, igk_k(ig,ik) )    ! k+G
      gk (2,ig) = xk(2, ik) + g(2, igk_k(ig,ik) )
      gk (3,ig) = xk(3, ik) + g(3, igk_k(ig,ik) )
      qg(ig) = gk(1, ig)**2 +  gk(2, ig)**2 + gk(3, ig)**2   ! (k+G)^2
   ENDDO
   ...
   DO iw = 1, n_proj
      ...
      ENDDO ! lm
      DO ig=1,npw
         ! zh_mo: when expand the Spherical wave in the basis of plane wave, 
         ! and when the spherical wave is not located at the orignal point of coordiate 
         ! system, we only need to move them entirely, and add the corresponding 
         ! phase factor before the plane wave
         iig = igk_k(ig,ik)
